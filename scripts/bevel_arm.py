...
class Bevel_arm(Node):
    def __init__(self):
        super().__init__("arm_drive")
        self.outbuff = [0] *6 
        
        self.bevel_pub = self.create_publisher(std_msgs.Int32MultiArray, '/stm_write', queue_size=10) ## check topic name later
        # rclpy.init_node('arm_drive')
        # rclpy.Subscriber('joy_arm', sensor_msgs.Joy, self.joyCallback)
        self.sub = self.create_subscription(sensor_msgs.Joy, '/joy_arm', self.joyCallback)  ## check topic name later
    ...
    def joyCallback (self, msg):
        ...
        self.get_logger().info(self.outbuff)
        msg_publish=self.createMsg(self.outbuff)
        self.bevel_pub.publish(msg_publish)
